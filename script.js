class CharacterGenerator {
  ...
  async loadResourcesFromJson() {
    try {
      ...
      const toWeights = (arr) =>
        Array.isArray(arr)
          ? arr.map((x) => Number(x?.possibility ?? 1) || 0)
          : [];
  ...
  async loadImages() {
    ...
    const skinKeys = this.keys?.skin?.length
      ? this.keys.skin
    ...
    // 얼굴 이미지 로드
    const faceKeys = this.keys?.face?.length
      ? this.keys.face
      : Array.from({ length: 9 }, (_, i) => String(i).padStart(3, "0"));
    ...
    // 얼굴2 이미지 로드
    const face2Keys = this.keys?.face2?.length
      ? this.keys.face2
    ...
    // 머리카락 이미지 로드
    const hairKeys = this.keys?.hair?.length
      ? this.keys.hair
      ...
      for (let x = 0; x < compositeCanvas.width; x++) {
        ...
        if (alpha > 0) {
          ...
          directions.forEach((dir) => {
            ...
            if (
              newX >= 0 &&
              newX < compositeCanvas.width &&
              newY >= 0 &&
              newY < compositeCanvas.height
            ) {
  ...
  downloadCharacter() {
    ...
    this.drawImageWithOutlineForDownload(
      tempCanvas,
      downloadCtx,
      centerX,
      centerY,
  ...
  drawImageWithOutlineForDownload(
    compositeCanvas,
    ctx,
    centerX,
    centerY,
